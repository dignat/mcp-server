...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (request.params.name === 'flow-chart') {
        let input;
        let output;
        const exists = await checkFile(request.params.arguments?.input);
        if (exists) {
            const inputParam = request.params.arguments?.input;
            const outputParam = request.params.arguments?.output;
