...
export async function createMermaidFile(fileName, dataToWrite) {
...
async function createFlowChartPng(mermaidCode, flowchartFileName) {
    let inputFileName = "";
    let outputFileName = "";
    let chunk = "";
    const checkFileExtension = mermaidCode.split(".").pop();
    const exists = await checkFile(mermaidCode);
    if (!exists) {
    ...
    if (mermaidCode.length) {
        await execAsync(`mmdc -i "${inputFileName}" -o "${outputFileName}" -t dark -b transparent`);
        //await run(inputFileName, outputFileName as `${string}.png` | `${string}.svg` | `${string}.pdf` | "/dev/stdout")
        console.log(` i am going to ex ecute ${inputFileName} and ${outputFileName}`);
    }
    else {
    ...
    if ((await checkFile(outputFileName)) ||
        ...
        throw new Error(`File was not created check the correct syntax for mermaid file ${inputFileName}`);
    }
}
...
console.log("\nConnected to server with prompts:", response.prompts.map((prompt) => prompt.name));
...
async function agentLoop() {
    ...
    const askQuestion = () => {
        rl.question("Chat with history: ", async (userInput) => {
            ...
            messages.push({ role: "user", content: userInput }, { role: "assistant", content: content });
            askQuestion();
            if (!toolCalls.length) {
                return;
            }
